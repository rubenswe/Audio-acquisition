...
class PlayRecorder():
    ...
    def __init__(self, input_file=None, output_file=None):  # data?
        ...
        """self.stream = self.p.open(format=pa.paFloat32,
                        channels=self.input_file.channels,  # What to do in the case of 1 channel for playing and 2 for recording? Use 2 channels always?(leaving one channel empty in the mono case if possible)
                        rate=self.input_file.samplerate,
                        frames_per_buffer=0, # What should be used?
                        ...
                        input=True,
                        output=True,
                        stream_callback=callback)  # Where to define callback() ?
        """
...
data = []
args = 0
pr_data = PlayRecorder.from_data(data, args)
